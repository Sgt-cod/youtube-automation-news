...
def gerar_roteiro(duracao_alvo, noticia=None):
    ...
    if noticia:
        prompt = f"""Crie um script para vídeo sobre esta notícia:
Título: {noticia['titulo']}
Requisitos: {tempo}, {palavras_alvo} palavras, tom noticioso, comece com "Atenção! Notícia importante...", finalize com "Fonte na descrição", apenas texto puro, sem formatação."""
    else:
        tema = random.choice(config['temas'])
        if duracao_alvo == 'short':
            prompt = f"""Crie um script de SHORT sobre: {tema}
Requisitos: {palavras_alvo} palavras, comece com "Você sabia que...", 1 curiosidade fascinante, finalize com "Incrível, né?", apenas texto puro."""
        else:
            prompt = f"""Crie um script sobre: {tema}
Requisitos: {tempo}, {palavras_alvo} palavras, comece com "Olá! Hoje vamos explorar...", 10-15 curiosidades, finalize com "Qual curiosidade mais te surpreendeu?", apenas texto puro."""
...
async def criar_audio_async(texto, output_file):
    ...
    for tentativa in range(3):
        try:
            ...
            print(f"✅ Edge TTS sucesso (tentativa {tentativa + 1})")
            return
...
def criar_audio(texto, output_file):
    ...
    try:
        ...
        print("⚠️ Áudio criado com gTTS")
        return output_file

def buscar_midia_pexels(palavras_chave, tipo='video', quantidade=1):
    headers = {'Authorization': PEXELS_API_KEY}
    traducoes = {
        'tecnologia': ['technology innovation', 'modern tech', 'digital future', 'ai robot'],
        'espaço': ['space galaxy', 'astronomy stars', 'cosmos universe', 'planets'],
        'oceano': ['ocean waves', 'underwater sea', 'marine life', 'coral reef'],
        'animais': ['wild animals', 'wildlife nature', 'safari', 'jungle forest'],
        'ciência': ['science lab', 'laboratory', 'experiment', 'research'],
        'natureza': ['nature landscape', 'mountain', 'waterfall', 'forest']
    }
    ...
    print(f"🔍 Buscando: '{palavra_busca}' (pág {pagina})")
    midias = []
    if tipo == 'video':
            ...
            if response.status_code == 200:
                ...
                for video in videos:
                    ...
                    if len(midias) >= quantidade:
                        break
                print(f"   ✓ {len(midias)} vídeos")
        except Exception as e:
            print(f"   ⚠️ Erro vídeos: {e}")
    if len(midias) < quantidade:
        orientacao = 'portrait' if VIDEO_TYPE == 'short' else 'landscape'
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   ✓ +{len(fotos[:quantidade * 2])} fotos")
        except Exception as e:
            print(f"   ⚠️ Erro fotos: {e}")
...
def main():
    ...
    if len(midias) < 3:
        ...
        midias.extend(buscar_midia_pexels(['nature'], tipo='foto', quantidade=5))
