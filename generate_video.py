...
Requisitos: {tempo}, {palavras_alvo} palavras, tom noticioso, comece com "Atenção!", finalize com "Fonte na descrição", texto puro sem formatação."""
    else:
        if duracao_alvo == 'short':
            prompt = f"""Crie script de SHORT sobre: {titulo}
Requisitos: {palavras_alvo} palavras, comece com "Você sabia que...", 1 curiosidade fascinante, finalize com "Incrível, né?", texto puro sem símbolos."""
        else:
            prompt = f"""Crie script sobre: {titulo}
Requisitos: {tempo}, {palavras_alvo} palavras, comece com "Olá! Hoje vamos explorar...", 10-15 curiosidades, finalize com "Qual te surpreendeu?", texto puro."""
